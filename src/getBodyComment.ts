...
export function getBodyComment({
  ...
  if (!errorsInModifiedFiles.length) {
    ...
    s += `**${errorsInModifiedFiles.length} ts error${
      errorsInModifiedFiles.length === 1 ? '' : 's'
    } detected in the modified files.**  \n`
    s += BLANK_LINE
  ...
  s += BLANK_LINE
  s +=
    '<a href="https://github.com/chefjackson/action-check-typescript"><sub>chefjackson/action-check-typescript</sub></a>'
